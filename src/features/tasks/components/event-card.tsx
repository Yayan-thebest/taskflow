...
export const EventCard = ({ id, title, assignee, project, status}: EventCardProps) => {
    ...
    return (
        <div className="px-2">
            <div onClick={onClick} className={cn("p-1.5 text-xs bg-white text-primary border rounded-md border-l-8 flex flex-col gap-y-1.5 cursor-pointer hover:opacity-75 transition", statusColorMap[status])}>
                <p>{title}</p>
